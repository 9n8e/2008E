char __cdecl RBX::Http::trustCheck(const char *Str2)
{
  ...
  boost::call_once(RBX::initTrustCheck, &flagTrustCheck);
  if ( skipTrustCheck )
    return 1;
  ...
  v3 = __mbscmp(*v2, (const unsigned __int8 *)Str2) == 0;
  v4 = v9.m_pszData - 16;
  if ( _InterlockedDecrement((volatile signed __int32 *)v9.m_pszData - 1) <= 0 )
    (*(void (__stdcall **)(char *))(**(_DWORD **)v4 + 4))(v4);
  if ( v3 || RBX::Http::isRobloxSite(Str2) || RBX::Http::isScript(Str2) )
    return 1;
  ATL::CUrl::CUrl(&v12);
  ATL::CUrl::CrackUrl(&v12, Str2, 0);
